-- Fix "column reference id is ambiguous": users is a view joining auth.users
-- and public tables, so qualify all references with public.users explicitly.

DROP FUNCTION IF EXISTS get_suggested_authors(uuid, text, text);
