...
export async function POST(request: NextRequest) {
  if (!process.env.CRON_SECRET) {
    console.error("[cron/article-type-scoring] CRON_SECRET not set");
  ...
  after(async () => {
    try {
      ...
      if (!res.ok) {
        console.error(`[cron/article-type-scoring] submit failed: ${res.status}`, JSON.stringify(result));
      } else {
        console.log(`[cron/article-type-scoring] mode=${mode} since=${since} → batchId=${result.batchId} count=${result.articleCount}`);
      }
    } catch (e) {
      console.error("[cron/article-type-scoring] submit threw:", e);
    }
  });
