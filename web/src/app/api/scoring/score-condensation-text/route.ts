...
async function condenseWithRetry(
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      return await scoreCondensation(article, specialty, activePrompt, "condensation_text");
    } catch (err: unknown) {
      const status = (err as { status?: number })?.status;
      if (status === 429 && attempt < retries - 1) {
        const waitMs = (attempt + 1) * 60_000;
        console.warn(`[scoring/score-condensation-text] rate limited — waiting ${waitMs / 1000}s before retry ${attempt + 1}/${retries - 1}`);
...
export async function POST(request: NextRequest) {
  ...
  const runId = await startScoringRun("condensation_text", specialty, activePrompt.version);
      ...
      try {
        await Promise.all(
          toScore.map((article) =>
            limiter(async () => {
              try {
                const condensation = await condenseWithDelay(article, specialty, activePrompt!);
                ...
                scored++;
                void logArticleEvent(article.id, "condensation_text_scored", {
                ...
                console.error(`[scoring/score-condensation-text] failed article ${article.id} (status=${status}): ${msg}`);
