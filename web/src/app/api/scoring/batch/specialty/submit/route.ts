...
import { prepareSpecialtyBatch, submitBatch } from "@/lib/scoring/batch/specialty-batch";
...
export async function POST(request: NextRequest) {
  ...
  let prepared;
  try {
    prepared = await prepareSpecialtyBatch(specialty, { limit, edat_from, edat_to });
  } catch (e) {
  ...
  const runId = await startScoringRun("specialty", specialty, activePrompt.version, `batch:${userId}`);
  ...
  if (insertError) {
    console.error("[batch/submit] scoring_batches insert failed:", insertError.message);
