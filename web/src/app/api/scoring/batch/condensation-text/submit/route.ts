...
export async function POST(request: NextRequest) {
  ...
  let prepared;
  try {
    prepared = await prepareCondensationTextBatch(specialty, { limit, edat_from, edat_to });
  } catch (e) {
  ...
  const runId = await startScoringRun("condensation_text", specialty, activePrompt.version, `batch:${userId}`);
