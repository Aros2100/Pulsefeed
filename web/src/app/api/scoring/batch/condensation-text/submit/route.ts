...
import { prepareCondensationTextBatch } from "@/lib/scoring/batch/condensation-text-batch";
...
export async function POST(request: NextRequest) {
  if (!process.env.CRON_SECRET) {
    console.error("[batch/condensation-text/submit] CRON_SECRET not set");
  ...
  let prepared;
  try {
    prepared = await prepareCondensationTextBatch(specialty, { limit, edat_from, edat_to, mode, since });
  } catch (e) {
  ...
  const runId = await startScoringRun("condensation_text", specialty, activePrompt.version, `batch:${triggeredBy}`, mode ?? null);
  ...
  if (insertError) {
    console.error("[batch/condensation-text/submit] scoring_batches insert failed:", insertError.message);
