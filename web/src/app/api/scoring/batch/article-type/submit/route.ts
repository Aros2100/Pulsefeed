...
export async function POST(request: NextRequest) {
  ...
  let prepared;
  try {
    prepared = await prepareArticleTypeBatch(specialty, { limit, edat_from, edat_to });
  } catch (e) {
  ...
  const runId = await startScoringRun("article_type_prod", specialty, activePrompt.version, `batch:${userId}`);
