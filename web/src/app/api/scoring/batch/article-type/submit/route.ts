...
import { prepareArticleTypeBatch } from "@/lib/scoring/batch/article-type-batch";
...
export async function POST(request: NextRequest) {
  if (!process.env.CRON_SECRET) {
    console.error("[batch/article-type/submit] CRON_SECRET not set");
  ...
  let prepared;
  try {
    prepared = await prepareArticleTypeBatch(specialty, { limit, edat_from, edat_to, mode, since });
  } catch (e) {
  ...
  const runId = await startScoringRun("article_type_prod", specialty, activePrompt.version, `batch:${triggeredBy}`, mode ?? null);
  ...
  if (insertError) {
    console.error("[batch/article-type/submit] scoring_batches insert failed:", insertError.message);
