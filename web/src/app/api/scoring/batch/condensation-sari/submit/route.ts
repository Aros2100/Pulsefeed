...
export async function POST(request: NextRequest) {
  ...
  let prepared;
  try {
    prepared = await prepareCondensationSariBatch(specialty, { limit, edat_from, edat_to });
  } catch (e) {
  ...
  const runId = await startScoringRun("condensation_sari", specialty, activePrompt.version, `batch:${userId}`);
