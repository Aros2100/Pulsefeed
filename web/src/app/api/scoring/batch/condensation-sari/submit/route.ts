...
import { prepareCondensationSariBatch } from "@/lib/scoring/batch/condensation-sari-batch";
...
export async function POST(request: NextRequest) {
  if (!process.env.CRON_SECRET) {
    console.error("[batch/condensation-sari/submit] CRON_SECRET not set");
  ...
  let prepared;
  try {
    prepared = await prepareCondensationSariBatch(specialty, { limit, edat_from, edat_to, mode, since });
  } catch (e) {
  ...
  const runId = await startScoringRun("condensation_sari", specialty, activePrompt.version, `batch:${triggeredBy}`, mode ?? null);
  ...
  if (insertError) {
    console.error("[batch/condensation-sari/submit] scoring_batches insert failed:", insertError.message);
