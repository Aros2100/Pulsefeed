...
export async function POST(request: NextRequest) {
  if (!process.env.CRON_SECRET) {
    console.error("[batch/article-geo-class-b/submit] CRON_SECRET not set");
  ...
  const { limit, specialty, edat_from, edat_to, mode, since } = result.data;
  const triggeredBy = isCronCall ? "cron" : (userId ?? "manual");

  let prepared;
  try {
    prepared = await prepareArticleGeoClassBBatch({ limit, specialty, edat_from, edat_to, mode, since });
  } catch (e) {
  ...
  if (insertError) {
    console.error("[batch/article-geo-class-b/submit] scoring_batches insert failed:", insertError.message);
