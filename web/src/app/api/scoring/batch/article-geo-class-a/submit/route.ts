...
export async function POST(request: NextRequest) {
  ...
  const { limit, edat_from, edat_to } = result.data;
  const userId = auth.userId;

  let prepared;
  try {
    prepared = await prepareArticleGeoClassABatch({ limit, edat_from, edat_to });
  } catch (e) {
  ...
  if (insertError) {
    console.error("[geo-class-a/submit] scoring_batches insert failed:", insertError.message);
