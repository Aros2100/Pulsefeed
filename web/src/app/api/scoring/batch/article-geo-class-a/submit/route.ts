...
import {
  prepareArticleGeoClassABatch,
  submitBatch,
} from "@/lib/scoring/batch/article-geo-class-a-batch";
...
export async function POST(request: NextRequest) {
  if (!process.env.CRON_SECRET) {
    console.error("[batch/article-geo-class-a/submit] CRON_SECRET not set");
  ...
  const { limit, edat_from, edat_to, mode, since } = result.data;
  const triggeredBy = isCronCall ? "cron" : (userId ?? "manual");

  let prepared;
  try {
    prepared = await prepareArticleGeoClassABatch({ limit, edat_from, edat_to, mode, since });
  } catch (e) {
  ...
  if (insertError) {
    console.error("[batch/article-geo-class-a/submit] scoring_batches insert failed:", insertError.message);
