...
interface Author    { lastName?: string; foreName?: string; affiliation?: string | null }
...
// Card components
function Card({ id, children, green }: { id?: string; children: React.ReactNode; green?: boolean }) {
  return (
    <div id={id} style={{
      background: "#fff",
      borderRadius: "10px",
...
export default async function ArticleDetailPage({
  ...
  const navItems = [
    ...
    meshTerms.length ? { id: "mesh", label: "MeSH Terms", badge: meshTerms.length } : null,
    isEnriched ? { id: "ai-summary", label: "AI Summary", ai: true } : null,
    isEnriched && pico ? { id: "pico", label: "PICO", ai: true } : null,
    authors.length ? { id: "authors", label: "Authors", badge: authors.length } : null,
    abstract ? { id: "abstract", label: "Abstract" } : null,
    grants.length ? { id: "funding", label: "Funding", badge: grants.length } : null,
  ...
  const factRows: FactRow[] = [
    ...
    article.volume ? fr("Volume / Issue", `${article.volume}${article.issue ? ` / ${article.issue}` : ""}`) : null,
    article.article_number ? fr("Article number", article.article_number) : null,
    authors.length ? fr("Authors", `${[authors[0].foreName, authors[0].lastName].filter(Boolean).join(" ")}${authors.length > 1 ? " et al." : ""}${authors[0].affiliation ? ` · ${authors[0].affiliation}` : ""}`) : null,
    fr("Imported", importedDisplay),
  ].filter((r): r is FactRow => r !== null);

  const factRows2: FactRow[] = [
    article.publication_types?.length ? fr("Publication type", article.publication_types[0]) : null,
    article.language ? fr("Language", LANGUAGE_NAMES[article.language] ?? article.language.toUpperCase()) : null,
    fr("PubMed", <a key="pm" href={pubmedUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#1a6eb5", textDecoration: "none" }}>PMID {article.pubmed_id} ↗</a>),
    (article.issn_electronic ?? article.issn_print) ? fr("ISSN", article.issn_electronic ?? article.issn_print) : null,
  ].filter((r): r is FactRow => r !== null);

  // Parse abstract sections (BACKGROUND, METHODS, etc.)
    ...
    <div style={{ fontFamily: "var(--font-inter), Inter, sans-serif", background: "#f5f7fa", color: "#1a1a1a", minHeight: "100vh" }}>
      ...
      <div style={{
        ...
        <main style={{ padding: "32px 0 80px" }}>
          ...
          <Card>
            <CardHeader
              label={topSpecialty ?? "Article"}
              right={
                <button style={{
                  fontSize: "12px", color: "#5a6a85", background: "#fff",
              ...
              <CardBody>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                  {article.keywords.map((kw, i) => (
                    <span key={i} style={{
                      fontSize: "13px", background: "#fff", border: "1px solid #ccc",
                      borderRadius: "4px", padding: "5px 12px", color: "#1a1a1a",
                    }}>
                      {kw}
                    </span>
                  ))}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  {[
                    ...
                    <div key={p.label} style={{
                      ...
                      padding: "14px", border: "1px solid #eef2f7",
                    }}>
                    ...
                    <div key={i} style={{ marginBottom: i < abstractSections.length - 1 ? "18px" : 0 }}>
                      {s.label && (
                        <span style={{
                          ...
                          textTransform: "uppercase", color: "#5a6a85", display: "block", marginBottom: "5px",
                        }}>
                          {s.label}
                        </span>
                      )}
          ...
          <Card id="citation">
            ...
            <CardBody>
              <div style={{
                fontSize: "13px", lineHeight: 1.6, color: "#444",
