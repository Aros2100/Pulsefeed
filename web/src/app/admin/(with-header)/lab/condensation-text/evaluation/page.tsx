    ...
    <div style={{ borderTop: "1px solid #f0f0f0", padding: "14px 24px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px" }}>
        ...
        {row.disagreement_reason && row.disagreement_reason !== "rejected" && (
          <span style={{ fontSize: "11px", fontWeight: 600, borderRadius: "4px", padding: "2px 7px", background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", flexShrink: 0, maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          ...
          {row.reject_reasons && row.reject_reasons.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: row.comment ? "6px" : 0 }}>
              {row.reject_reasons.map((r) => (
                <span key={r} style={{ fontSize: "11px", fontWeight: 600, borderRadius: "4px", padding: "2px 7px", background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca" }}>
                  {r}
                </span>
              ))}
            </div>
      ...
      {article && (article.short_headline || article.short_resume || article.bottom_line) && (
        <div style={{ marginTop: "10px", padding: "10px 12px", background: "#fef2f2", borderRadius: "6px", borderLeft: "3px solid #fecaca" }}>
          {article.short_headline && <div style={{ fontSize: "13px", fontWeight: 700, color: "#1a1a1a", marginBottom: "4px" }}>{article.short_headline}</div>}
          {article.short_resume && <div style={{ fontSize: "12px", color: "#444", lineHeight: 1.5, marginBottom: article.bottom_line ? "6px" : 0 }}>{article.short_resume}</div>}
          {article.bottom_line && <div style={{ fontSize: "12px", color: "#666", fontStyle: "italic", lineHeight: 1.4 }}>{article.bottom_line}</div>}
        </div>
      )}

      {abstract && (
        <details style={{ marginTop: "8px" }}>
          <summary style={{ fontSize: "12px", color: "#5a6a85", cursor: "pointer", userSelect: "none", listStyle: "none" }}>▶ Show abstract</summary>
...
export default async function CondensationTextEvaluationPage({ searchParams }: Props) {
  ...
  const [articlesForMap, allAccRes] = await Promise.all([
    articleIds.length > 0
      ? admin.from("articles")
          .select("id, title, journal_abbr, abstract, short_headline, short_resume, bottom_line")
          .in("id", articleIds)
    ...
    <div style={{ fontFamily: "var(--font-inter), Inter, sans-serif", background: "#f5f7fa", color: "#1a1a1a", minHeight: "100vh" }}>
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div style={{ marginBottom: "8px" }}>
          <Link href="/admin/lab/condensation-text" style={{ fontSize: "13px", color: "#5a6a85", textDecoration: "none" }}>
            ← Text Condensation
          </Link>
        </div>
        ...
        <div style={{ marginBottom: "20px" }}>
          <div style={{ fontSize: "11px", letterSpacing: "0.08em", color: "#059669", textTransform: "uppercase", fontWeight: 700, marginBottom: "6px", display: "flex", alignItems: "center", gap: "8px" }}>
            <span>Prompt Evaluation · Text Condensation</span>
            {selectedVersion && (
          ...
          <p style={{ fontSize: "13px", color: "#888", marginTop: "6px" }}>
            Tilfælde hvor AI-genereret tekst blev afvist — brug disse til at forbedre prompten
          </p>
        </div>

        {/* Data banner */}
        <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: "8px", padding: "10px 16px", marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#15803d", flexShrink: 0, display: "inline-block" }} />
            <span style={{ fontSize: "13px", color: "#14532d", fontWeight: 500 }}>{totalRejected} afvisninger</span>
          </div>
          <Link
            href="/admin/lab/condensation-text/optimize"
        ...
        <div style={card}>
          <div style={{ background: "#EEF2F7", borderBottom: "1px solid #dde3ed", padding: "10px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: "11px", letterSpacing: "0.08em", color: "#dc2626", textTransform: "uppercase", fontWeight: 700 }}>Afvist AI-tekst</span>
            <span style={{ fontSize: "11px", fontWeight: 700, background: "#dc2626", color: "#fff", borderRadius: "4px", padding: "2px 8px" }}>{rejections.length}</span>
          </div>
          {rejections.length === 0 ? (
            <div style={{ padding: "24px", fontSize: "14px", color: "#888" }}>
              Ingen afvisninger — alt AI-genereret tekst er godkendt
            </div>
          ) : (
            rejections.map((row) => (
              <ArticleRow
