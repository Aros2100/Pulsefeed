...
export default async function CondensationTextSimulatePage({ searchParams }: Props) {
  const { run_id } = await searchParams;

  if (!run_id) redirect("/admin/lab/condensation-text/optimize");
  ...
  if (!run) redirect("/admin/lab/condensation-text/optimize");
  ...
  for (const d of decisions) {
    if (d.decision === "rejected" && d.article_id && d.articles?.title && !seenIds.has(d.article_id)) {
  ...
  for (const d of agreementData) {
    if (d.decision === d.ai_decision && d.decision === "approved" && d.article_id && d.articles?.title && !seenIds.has(d.article_id)) {
      seenIds.add(d.article_id);
