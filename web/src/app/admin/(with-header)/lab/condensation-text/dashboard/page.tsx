...
export default async function CondensationTextDashboardPage() {
  ...
  return (
    <div style={{ fontFamily: "var(--font-inter), Inter, sans-serif", background: "#f5f7fa", color: "#1a1a1a", minHeight: "100vh" }}>
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        {/* Bar chart */}
        <div style={card}>
          <div style={cardHeader}>Godkendelsesrate pr. version — Tekst</div>
            ...
            {chartVersions.length === 0 ? (
              ...
              <>
                <div style={{ display: "flex", gap: "0" }}>
                  <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: `${barHeight}px`, paddingRight: "10px", flexShrink: 0 }}>
                    {[100, 80, 60, 40, 20, 0].map((tick) => (
                      <div key={tick} style={{ fontSize: "10px", color: "#aaa", textAlign: "right", width: "28px", lineHeight: "1" }}>{tick}%</div>
                    ))}
                  </div>
                  <div style={{ flex: 1, display: "flex", alignItems: "flex-end", gap: "16px", justifyContent: "center", height: `${barHeight}px`, borderLeft: "1px solid #e8ecf1", borderBottom: "1px solid #e8ecf1", position: "relative", paddingBottom: "1px" }}>
                    {[20, 40, 60, 80].map((tick) => (
                      <div key={tick} style={{ position: "absolute", bottom: `${tick}%`, left: 0, right: 0, borderTop: "1px dashed #f0f2f5" }} />
                    ))}
                    {chartVersions.map((v) => {
                      const acc = v.accuracy ?? 0;
                      const h = Math.max((acc / 100) * barHeight, 4);
                      return (
                        <div key={v.version} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, maxWidth: "80px", position: "relative", zIndex: 1 }}>
                          <div style={{ fontSize: "14px", fontWeight: 800, color: accColor(v.accuracy), marginBottom: "6px" }}>
                            {v.accuracy != null ? `${v.accuracy}%` : "—"}
                          </div>
                          <div style={{ width: "100%", height: `${h}px`, borderRadius: "6px 6px 2px 2px", background: v.active ? "linear-gradient(180deg, #059669 0%, #047857 100%)" : "#EEF2F7", border: v.active ? "2px solid #059669" : "1px solid #dde3ed" }} />
                        </div>
                      );
                    })}
                  </div>
                ...
                <div style={{ display: "flex", gap: "0", marginTop: "8px" }}>
                  ...
                  <div style={{ flex: 1, display: "flex", gap: "16px", justifyContent: "center" }}>
                    {chartVersions.map((v) => (
                      <div key={v.version} style={{ flex: 1, maxWidth: "80px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: "3px" }}>
                        <div style={{ fontSize: "12px", fontWeight: 700, color: v.active ? "#059669" : "#5a6a85" }}>{v.version}</div>
                        {v.active && (
                          <span style={{ fontSize: "9px", fontWeight: 700, background: "#059669", color: "#fff", borderRadius: "3px", padding: "1px 5px" }}>Aktiv</span>
                        )}
                      </div>
                    ))}
                  </div>
        ...
        <div style={card}>
          <div style={cardHeader}>Versions-sammenligning — Tekst</div>
          {tableRows.length === 0 ? (
