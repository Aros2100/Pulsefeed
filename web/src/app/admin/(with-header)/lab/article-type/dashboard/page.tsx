...
export default async function ArticleTypeDashboardPage() {
  ...
  // Paginate all lab_decisions for article_type
  type Decision = {
  ...
  const allVersions: VersionStats[] = uniqueVersions.map((v, i) => {
    ...
    const corrected = ds.filter((d) => d.ai_decision !== d.decision).length;
    ...
    // Top correction patterns: "AI: X → Human: Y"
    const reasonMap: Record<string, number> = {};
    for (const d of ds) {
      if (d.ai_decision !== d.decision) {
        const r = `${d.ai_decision} → ${d.decision}`;
    ...
    <div style={{ fontFamily: "var(--font-inter), Inter, sans-serif", background: "#f5f7fa", color: "#1a1a1a", minHeight: "100vh" }}>
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div style={{ marginBottom: "24px" }}>
          <div style={{ fontSize: "11px", letterSpacing: "0.08em", color: "#7c3aed", textTransform: "uppercase", fontWeight: 700, marginTop: "16px", marginBottom: "6px" }}>
            Performance
          </div>
        ...
        {/* Vertical bar chart */}
        <div style={card}>
          <div style={cardHeader}>Nøjagtighed pr. version — Artikel Type</div>
              ...
              <>
                ...
                <div style={{ display: "flex", gap: "20px", justifyContent: "center", marginTop: "16px", fontSize: "11px", color: "#5a6a85" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                    <span style={{ display: "inline-block", width: "12px", height: "12px", background: "linear-gradient(180deg, #7c3aed, #6d28d9)", borderRadius: "3px" }} />
                    Aktiv version
                  </span>
                  <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
        ...
        <div style={card}>
          <div style={cardHeader}>Versions-sammenligning — Artikel Type</div>
          {tableRows.length === 0 ? (
