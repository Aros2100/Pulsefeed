...
function ArticleRow({ row, article }: { row: DisagreementRow; article: ArticleDetail | undefined }) {
  ...
  const aiTags    = row.ai_decision ? [row.ai_decision] : [];
  const humanTags = row.decision    ? [row.decision]    : [];
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: "12px", color: "#888", marginTop: "3px" }}>
          ...
          {row.ai_confidence != null && (
            <span style={{ marginLeft: "8px", fontSize: "11px", color: "#5a6a85" }}>
...
export default async function ArticleTypeEvaluationPage({ searchParams }: Props) {
  ...
  const [articlesForMap, allAccRes] = await Promise.all([
    ...
    admin.from("lab_decisions")
      ...
      .eq("specialty", FIXED_SPECIALTY).eq("module", "article_type")
      .not("ai_decision", "is", null)
      .limit(10000),
  ]);
  ...
  const dataBanner = totalDisagree < ARTICLE_TYPE_DISAGREEMENT_THRESHOLD
    ? { bg: "#fef2f2", border: "#fecaca", dot: "#dc2626", text: "#b91c1c", msg: `Insufficient data — need at least ${ARTICLE_TYPE_DISAGREEMENT_THRESHOLD} disagreements to identify reliable trends (${totalDisagree} so far)` }
    ...
    <div style={{ fontFamily: "var(--font-inter), Inter, sans-serif", background: "#f5f7fa", color: "#1a1a1a", minHeight: "100vh" }}>
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div style={{ marginBottom: "8px" }}>
          <Link href="/admin/lab/article-type" style={{ fontSize: "13px", color: "#5a6a85", textDecoration: "none" }}>
            ← Artikel Type
          </Link>
        </div>
        ...
        <div style={{ marginBottom: "20px" }}>
          <div style={{ fontSize: "11px", letterSpacing: "0.08em", color: "#7c3aed", textTransform: "uppercase", fontWeight: 700, marginBottom: "6px" }}>
            Prompt Evaluation · Article Type
          </div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
            <h1 style={{ fontSize: "22px", fontWeight: 700, margin: 0 }}>
              AI/Human Disagreements · Artikel Type
            </h1>
            <PromptDrawer
          ...
          <p style={{ fontSize: "13px", color: "#888", marginTop: "6px" }}>
            Cases where AI and human article type classifications differed — use these to refine the prompt
          </p>
        </div>
        ...
        <div style={{ background: dataBanner.bg, border: `1px solid ${dataBanner.border}`, borderRadius: "8px", padding: "10px 16px", marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
          ...
          {hasSufficientData ? (
            ...
            <span
              title={`Need at least ${ARTICLE_TYPE_DISAGREEMENT_THRESHOLD} disagreements first`}
        ...
        <div style={card}>
          <CardHeader label="AI type → Human type" count={totalCorrected} color="#d97706" />
          {totalCorrected === 0 ? (
