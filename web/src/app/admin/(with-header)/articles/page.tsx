...
export default async function AdminArticlesPage() {
  ...
  const [subspecialties, specialtiesData] = await Promise.all([
    ...
    (admin as any).rpc("get_distinct_specialties"),
  ]);
    ...
    <div style={{ fontFamily: "var(--font-inter), Inter, sans-serif", background: "#f5f7fa", minHeight: "100vh" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <h1 style={{ fontSize: "22px", fontWeight: 700, margin: 0 }}>Articles</h1>
          <p style={{ fontSize: "13px", color: "#888", marginTop: "6px" }}>
            Browse and filter articles by date, subspecialty and article type
          </p>
        </div>

        <AdminArticleListClient subspecialties={subspecialties} specialties={specialties} />
      </div>
    </div>
  );
}
