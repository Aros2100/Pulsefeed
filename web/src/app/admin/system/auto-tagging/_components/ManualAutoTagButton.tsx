...
import { triggerAutoTag } from "./triggerAutoTag";

export function ManualAutoTagButton({ job }: { job: "specialty" | "article-type" }) {
  ...
  async function handleClick() {
    ...
    setResult(null);
    try {
      const data = await triggerAutoTag(job);
  ...
  return (
    <div style={{ marginTop: "16px" }}>
      ...
      >
        {state === "running" ? "Running…" : "Run auto-tag now"}
      </button>
      {state === "ok" && (
