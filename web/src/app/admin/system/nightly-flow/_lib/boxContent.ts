...
export function getBoxContent(id: string, box: ComponentBox): BoxContent {
  ...
  switch (id) {
    ...
    case "daily_author_update": return {
      ...
      counts: `${d(box).processed} processed · A=${d(box).scenario_a} B=${d(box).scenario_b} C=${d(box).scenario_c}`,
      extra: d(box).unmatched > 0 ? `${d(box).unmatched} unmatched` : "",
    };
    case "specialty_new": return {
      explanation: "is article in our specialty?",
      counts: `${d(box).scored} scored · ${d(box).failed} failed${d(box).approved !== undefined ? ` · ${d(box).approved} in · ${d(box).rejected} out` : ""}`,
    ...
    case "auto_tag_article_type": return {
      ...
      counts: `${nullStr(d(box).evaluated)} evaluated · ${nullStr(d(box).scored)} scored · ${nullStr(d(box).skipped)} skipped · ${d(box).approved} approved`,
      extra: "",
    };
    ...
    case "ready_by_morning": return {
      ...
      extra: `Total in system: ${Number(d(box).total_in_system).toLocaleString()} · prev night: ${Number(d(box).previous_night_total).toLocaleString()} (+${d(box).delta})`,
    };
